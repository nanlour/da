...
func TestDBManager(t *testing.T) {
    ...
    dbManager, err := util.NewDB(tempDir)
    if err != nil {
    ...
    err = dbManager.Insert(key, value)
    if err != nil {
        t.Errorf("Insert failed: %v", err)
    }

    // Test Get
    retrievedValue, err := dbManager.Get(key)
    if err != nil {
    ...
    err = dbManager.BatchInsert(batch)
    if err != nil {
        t.Errorf("BatchInsert failed: %v", err)
    }

    // Verify BatchInsert
    retrievedBatchValue, err := dbManager.Get([]byte("batchKey1"))
